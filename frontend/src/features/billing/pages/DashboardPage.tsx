...
// Design tokens — "Heritage" theme
const HERITAGE_THEME: CSSProperties = {
  ...
  '--color-border': 'rgba(26, 28, 30, 0.12)',
...
const KPI_CARDS = [
  { id: 'target_udara', key: 'billing.targetAir', value: '4 List', desc: 'Kiki: 4 | Yati: 0', icon: Plane, color: 'text-[#C99A2E]' },
  { id: 'target_laut', key: 'billing.targetSea', value: '52 List', desc: 'Thara: 48 | Rico: 2 | Ferly: 2', icon: Ship, color: 'text-[#1E5D54]' },
  { id: 'inv_month', key: 'billing.monthInvoices', value: '946', desc: '↘ 81.6% vs bulan lalu', subDesc1: '223', subDesc2: '723', icon: CalendarDays, color: 'text-[#3E5C76]' },
  { id: 'amt_month', key: 'billing.monthAmount', value: 'Rp. 10.837.404.519,95', desc: '↘ 83.7% vs bulan lalu', subDesc1: 'Rp 2,26 M', subDesc2: 'Rp 8,57 M', icon: CreditCard, color: 'text-[#963737]' },
]
...
export default function BillingDashboardPage() {
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-stretch gap-3 overflow-x-auto snap-x snap-mandatory pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:overflow-visible [&::-webkit-scrollbar]:hidden"
        style={{ scrollbarWidth: 'none' }}
      >
        {KPI_CARDS.map((card) => (
          <div key={card.id} className="relative group bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-[var(--color-primary)] font-[var(--font-display)] mb-1">{card.value}</h3>
            <p className="text-[10px] sm:text-xs text-[var(--color-secondary)]">{card.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-sm p-4 sm:p-6 mb-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
          ...
          {(activeTab === 'karyawan' || activeTab === 'sj-vs-bill') && (
            <div className="flex items-center gap-3 self-end sm:self-auto animate-fadeIn">
              ...
              <select
                ...
                className="h-9 rounded-lg border border-[var(--color-border)] bg-[var(--color-neutral)] hover:bg-[var(--color-surface)] px-3 py-1.5 text-sm font-semibold text-[var(--color-primary)] shadow-sm focus:border-[var(--color-tertiary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-tertiary)]/20 transition-all cursor-pointer"
              >
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-6" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          {activeTab === 'harian' && (
            <>
