...
export function HistoryPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 w-full min-w-0 space-y-6 sm:space-y-8 bg-[var(--color-surface)] font-[var(--font-body)] animate-fadeIn pb-24">
      {/* Page Header */}
      <PageHeader
        title={t('customerPriceList.historyTitle')}
        subtitle={
          loading
            ? t('common.loading')
            : `Customer: ${custCode ?? ''} ${custName ? `— ${custName}` : ''} · Total ${total} file price list.`
        }
        breadcrumbs={[
            ...
            <div className="px-6 py-16 text-center">
              <div className="flex flex-col items-center justify-center max-w-md mx-auto">
                ...
                <p className="text-[var(--color-secondary)] text-sm leading-relaxed mb-4">
                  File Excel price list khusus customer ini akan tersimpan di sini secara rapi.
                </p>
                <Link to={ROUTES.CUSTOMER_PRICE_LIST_UPLOAD}>
                  <Button variant="secondary" size="sm">
                    <Upload className="w-4 h-4 mr-1.5" /> Upload Price List
                  </Button>
                </Link>
              </div>
            </div>
            ...
            <div className="divide-y divide-[var(--color-border)]">
              {groups.map((group, gi) => (
                <div
                  ...
                  className={`p-3 ${gi % 2 === 1 ? 'bg-[var(--color-neutral)]/50' : 'bg-[var(--color-surface)]'}`}
                >
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          <div className="flex items-center gap-1.5 min-w-0">
                            {row.isSuperseded && (
                              <CornerDownRight size={13} className="text-[var(--color-secondary)]/40 shrink-0" />
                            )}
                            <span className="text-[0.85rem] font-mono text-[var(--color-secondary)] truncate">
                              {row.fileName}
                            </span>
                          </div>
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center justify-center max-w-md mx-auto">
                      ...
                      <Link to={ROUTES.CUSTOMER_PRICE_LIST_UPLOAD}>
                        <Button variant="secondary" size="sm">
                          <Upload size={16} className="mr-1.5" /> Upload Price List
                        </Button>
                      </Link>
                    </div>
                  </td>
              ...
              groups.map((group, gi) => (
                <tbody
                  ...
                  className={`divide-y divide-[var(--color-border)] ${
                    gi % 2 === 1 ? 'bg-[var(--color-neutral)]/40' : 'bg-[var(--color-surface)]'
                  }`}
                >
                  {group.items.map((row, ri) => {
                    ...
                    return (
                      <tr
                        key={row.id}
                        className={`hover:bg-[var(--color-neutral)] transition-colors duration-150 ${
                          row.isSuperseded ? 'opacity-55' : ''
                        ...
                        >
                          <div
                            className={`flex items-center gap-1.5 min-w-0 ${
                              !isHead
                                ? 'pl-3 border-l-2 border-dashed border-[var(--color-border)]'
                                : ''
                            }`}
                          >
                            {!isHead && (
                              <CornerDownRight
                                size={13}
                                className="text-[var(--color-secondary)]/40 shrink-0"
                              />
                            )}
      ...
      {editingUpload && (
        <EditEffectiveDateModal
          ...
          onSave={async (newDate) => {
            await customerPriceListApi.updateEffectiveDate(editingUpload.id, newDate)
            reload()
          }}
        />
      )}
      ...
      {selectedUploadForMarking && (
        <MarkingManagerModal
          ...
          uploadDescription={{
            ...
            custCode: selectedUploadForMarking.fdCustCode,
            custName,
          }}
          initialMarkings={selectedUploadForMarking.markings || []}
          onSave={async (markings) => {
            await customerPriceListApi.setUploadMarkings(selectedUploadForMarking.id, markings)
            reload()
          }}
        />
      )}
