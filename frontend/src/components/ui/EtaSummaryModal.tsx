...
export function EtaSummaryModal({ isOpen, onClose, data }: EtaSummaryModalProps) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-[var(--color-border)] flex items-center justify-between bg-amber-50/10">
          <div>
            <h2 className="text-xl font-bold font-[var(--font-display)] text-[var(--color-primary)] flex items-center gap-2">
              <Clock className="w-5 h-5 text-amber-500" /> Detil Consignee
            </h2>
            <p className="text-sm text-[var(--color-secondary)] mt-1">ETA sudah lewat, belum exit gudang</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 pt-6 pb-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-[var(--color-neutral)] rounded-lg p-3 border border-[var(--color-border)]">
            <p className="text-xs font-medium text-[var(--color-secondary)] mb-1">Total batch</p>
            <p className="text-2xl font-bold text-[var(--color-primary)]">{stats.totalBatches}</p>
          </div>
          <div className="bg-[var(--color-neutral)] rounded-lg p-3 border border-[var(--color-border)]">
            <p className="text-xs font-medium text-[var(--color-secondary)] mb-1">Grup consignee</p>
          ...
          <div className="bg-[var(--color-neutral)] rounded-lg p-3 border border-[var(--color-border)] relative overflow-hidden">
            ...
            <p className="text-xs font-medium text-[var(--color-secondary)] mb-1">Kritis (≥30 hari)</p>
            <p className="text-2xl font-bold text-rose-600">{stats.criticalCount}</p>
          </div>
          <div className="bg-[var(--color-neutral)] rounded-lg p-3 border border-[var(--color-border)]">
            <p className="text-xs font-medium text-[var(--color-secondary)] mb-1">Tertua</p>
            <p className="text-2xl font-bold text-[var(--color-primary)] flex items-baseline gap-1">
              {stats.maxAging} <span className="text-sm font-normal text-[var(--color-secondary)]">hari</span>
            </p>
          </div>
        </div>
            ...
            <div className="flex bg-[var(--color-neutral)] border border-[var(--color-border)] rounded-md p-0.5">
              <button
                ...
                className={cn(
                  ...
                  sortBy === 'usia' ? "bg-[var(--color-primary)] text-white" : "text-[var(--color-secondary)] hover:text-[var(--color-primary)]"
                )}
              >
                Usia
              </button>
                    ...
                    {displayCodes.map((c, cIdx) => (
                      ...
                      >
                        ...
                        <span className={cn(
                          ...
                          c.aging >= 30 ? "bg-rose-200" : c.aging >= 8 ? "bg-amber-200" : "bg-[var(--color-border)]"
                        )}>
                          {c.aging}d
                        </span>
                      </div>
