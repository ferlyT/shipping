...
import { X, Box, MapPin, Calendar, CheckCircle2, Activity, Truck, Ship, LogOut, Clock, AlertTriangle, Info, ChevronDown, ChevronRight, Eye, ClipboardList, Search, Rows3, LayoutGrid, PackageX, Book, Shirt, Settings, Cpu, Coffee, Pill as PillIcon, Car, Package, Plane, List, RotateCcw } from 'lucide-react'
  ...
  return (
    <tr className="bg-white hover:bg-[#EFF6FF] transition-colors duration-200 border-b border-[#E4E1DA] last:border-0">
      <td className="py-4 pl-4 pr-3">
        ...
        <span className="mt-1.5 inline-block"><StatusBadge status={row.fdStatus} exitDate={row.fdExitDate} /></span>
      </td>
      ...
      <td className="py-4 px-3 text-sm sm:text-[14.5px] md:text-[15px] text-[var(--color-secondary)]">
        ...
        {row.fdListType !== 1 && (row.fdListType === 2 || (row.fdContNo && row.fdContNo.trim() !== '')) && (
          <p className="text-[11px] sm:text-[11.5px] md:text-xs mt-1">Cont: {row.fdContNo || '—'}{row.fdContSize && row.fdContSize.trim() !== '' ? ` (${row.fdContSize.trim()})` : ''}</p>
        )}
        {row.fdKet && row.fdKet.trim() !== '' && (
          <p className="text-[11px] sm:text-[11.5px] md:text-xs mt-1 text-[var(--color-tertiary)] truncate max-w-[150px]" title={row.fdKet}>Ket: {row.fdKet}</p>
        )}
      </td>
      <td className="py-4 px-3">
...
function GroupSection({ viewMode, 
  ...
  // BUG FIX: markingApi.getGroups() kadang mengembalikan count/totalPkgs/totalWeight = 0
  // walau baris data sebenarnya ada (lihat markingApi.list() yang jadi sumber `total`).
  // Selama grup ini sudah dibuka dan list-nya sudah kebaca, pakai `total` (akurat, ikut
  // filter search/listType/isClosed yang sama) sebagai fallback biar badge count gak nampilin 0 palsu.
  const displayCount = groupMeta.count > 0 ? groupMeta.count : (open ? total : groupMeta.count);
  ...
  return (
    <div className="border-b border-[#E4E1DA] last:border-0">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-3 bg-[#F7F5F2]/95 px-4 sm:px-6 py-3 text-left transition-colors hover:bg-[#F7F5F2] border-y border-[#E4E1DA]"
      >
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full bg-white border border-[#E4E1DA] text-[10.5px] sm:text-[11px] font-bold text-[var(--color-secondary)] shrink-0">
            {displayCount}
          </span>
        </span>
...
const StatusBlock = ({
  ...
  // BUG FIX: markingApi.getGroups() kadang balikin count = 0 di semua grup padahal batch-nya
  // beneran ada (row-level list tetap kebaca normal). Kalau itu terjadi dan kita punya angka KPI
  // (totalCountOverride) yang > 0, pakai itu daripada nampilin "0 batch" yang jelas keliru.
  ...
  return (
    <div className={cn(
      "overflow-hidden rounded-[var(--radius-lg)] border border-[#E4E1DA] bg-white shadow-sm border-l-[3px]",
      status === 'open' ? "border-l-[var(--color-warning)]" : "border-l-[var(--color-success)]"
    )}>
      <button
        onClick={() => setExpanded((v) => !v)}
        className="flex w-full items-center justify-between gap-3 px-4 sm:px-6 py-3.5 sm:py-4 text-left bg-white transition-colors hover:bg-[#FAFAF8]"
      >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
              <div className="relative flex-1 sm:max-w-[280px]">
                <Search size={16} className="absolute left-3 sm:left-[14px] top-1/2 -translate-y-1/2 text-[var(--color-secondary)]" />
                <input
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Cari marking code, BL/AWB..."
                  className="w-full pl-9 sm:pl-[40px] pr-[30px] py-2.5 sm:py-[10px] rounded-[var(--radius-md)] border border-[#E4E1DA] bg-[#FAFAF8] text-[14px] sm:text-[14.5px] md:text-[15px] text-[var(--color-primary)] font-[var(--font-body)] outline-none focus:bg-white focus:border-[var(--color-tertiary)] focus:ring-2 focus:ring-[var(--color-tertiary)]/15 transition-all"
                />
                {search && (
