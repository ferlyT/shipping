...
import { X, Box, MapPin, Calendar, CheckCircle2, Activity, Truck, Ship, Warehouse, LogOut, Clock, AlertTriangle, Info, ChevronDown, ChevronRight, Eye, ListFilter, ClipboardList, Search, Rows3, LayoutGrid, PackageX, Book, Shirt, Settings, Cpu, Coffee, Pill as PillIcon, Car, Package, Plane } from 'lucide-react'
  ...
  return (
    <tr className="bg-white hover:bg-[#EFF6FF] transition-colors duration-200 border-b border-[#E4E1DA] last:border-0">
      <td className="py-3 pl-4 pr-3">
        ...
        <ActiveBadge active={row.fdStatus === 1} />
      </td>
      ...
      <td className="py-3 px-3 text-sm text-[var(--color-secondary)]">
        ...
        {row.fdListType !== 1 && (row.fdListType === 2 || (row.fdContNo && row.fdContNo.trim() !== '')) && (
          <p className="text-[11px] mt-0.5">Cont: {row.fdContNo || '—'}{row.fdContSize && row.fdContSize.trim() !== '' ? ` (${row.fdContSize.trim()})` : ''}</p>
        )}
        {row.fdKet && row.fdKet.trim() !== '' && (
          <p className="text-[11px] mt-0.5 text-[var(--color-tertiary)] truncate max-w-[150px]" title={row.fdKet}>Ket: {row.fdKet}</p>
        )}
      </td>
      <td className="py-3 px-3">
        <p className="text-sm text-[var(--color-primary)] font-medium">{row.fdJmlPack != null ? Number(row.fdJmlPack).toLocaleString('en-US') : 0} PKGS</p>
      ...
      <td className="py-3 px-3 text-sm whitespace-nowrap">
        <ExitBadge exitDate={row.fdExitDate} />
      </td>
      <td className="py-3 pr-4 pl-3 text-right whitespace-nowrap">
        <div className="flex items-center justify-end gap-1.5">
          <button
            onClick={() => onViewManifest(row)}
            className="inline-flex items-center gap-1.5 rounded-lg border border-[#E4E1DA] px-3 py-1.5 text-xs font-semibold text-[var(--color-primary)] transition-colors hover:bg-[#F7F5F2] bg-[var(--color-surface)]"
          >
...
function BatchCard({ row, onView, onViewManifest }: { row: Marking; onView: (row: Marking) => void, onViewManifest: (row: Marking) => void }) {
  return (
    <div className="bg-white border border-[#E4E1DA] rounded-[8px] overflow-hidden hover:shadow-md transition-all flex flex-col">
      {/* Header */}
      <div className="px-4 py-3 flex items-center justify-between bg-[var(--color-neutral)] border-b border-[#E4E1DA] gap-3">
        <div className="min-w-0">
          <div className="font-['Space_Grotesk'] text-[0.6rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mb-[3px]">Marking Code</div>
          <div className="font-['Fraunces'] font-semibold text-[1rem] tracking-[-0.01em] text-[var(--color-primary)] truncate">{row.fdMarkingCode}</div>
        </div>
      ...
      <div className="pt-1 px-4">
        <div className="flex flex-col items-start gap-[4px] py-3 border-b border-[#EFEDE7]">
          <div className="font-['Space_Grotesk'] text-[0.62rem] tracking-[0.06em] uppercase text-[var(--color-secondary)] shrink-0">Consignee</div>
          <div className="font-['Fraunces'] font-semibold text-[0.95rem] tracking-[-0.01em] text-[var(--color-primary)] leading-tight">{row.fdConsignee || '—'}</div>
          <div className="flex items-center gap-1 text-[11px] text-[var(--color-secondary)]">
            <MapPin className="h-3 w-3 shrink-0" /> <span className="truncate max-w-[220px]">{row.fdWilayah || 'Tidak diketahui'}</span>
          </div>
        </div>

        <div className="flex items-center justify-between gap-3 py-3 border-b border-[#EFEDE7]">
          <div className="font-['Space_Grotesk'] text-[0.62rem] tracking-[0.06em] uppercase text-[var(--color-secondary)] shrink-0">
            {row.fdListType === 1 ? 'AWB' : 'BL'}
          </div>
          <div className="text-[0.85rem] font-semibold text-[var(--color-primary)] text-right truncate">
            {row.fdListType === 1 ? (row.fdAWB || '—') : (row.fdBLNo || '—')}
          </div>
        </div>

        {row.fdListType !== 1 && (row.fdListType === 2 || (row.fdContNo && row.fdContNo.trim() !== '')) && (
          <div className="flex items-center justify-between gap-3 py-3 border-b border-[#EFEDE7]">
            <div className="font-['Space_Grotesk'] text-[0.62rem] tracking-[0.06em] uppercase text-[var(--color-secondary)] shrink-0">Container</div>
            <div className="text-[0.85rem] font-semibold text-[var(--color-primary)] text-right truncate">
              {row.fdContNo || '—'}{row.fdContSize && row.fdContSize.trim() !== '' ? ` (${row.fdContSize.trim()})` : ''}
            </div>
          </div>
        )}

        <div className="flex items-center py-3 border-b border-[#EFEDE7]">
          <div className="grid grid-cols-2 w-full gap-2">
            <div className="flex flex-col items-center text-center gap-[4px] border-r border-[#EFEDE7]">
              <div className="font-['Space_Grotesk'] text-[0.58rem] tracking-[0.06em] uppercase text-[var(--color-secondary)]">Pkg</div>
              <div className="text-[0.82rem] font-bold text-[var(--color-tertiary)]">{row.fdJmlPack != null ? Number(row.fdJmlPack).toLocaleString('en-US') : 0}</div>
            </div>
            <div className="flex flex-col items-center text-center gap-[4px]">
              <div className="font-['Space_Grotesk'] text-[0.58rem] tracking-[0.06em] uppercase text-[var(--color-secondary)]">Wgt</div>
              <div className="text-[0.82rem] font-bold text-[var(--color-primary)]">{row.fdJmlBerat != null ? Number(row.fdJmlBerat).toLocaleString('en-US') : 0} kg</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5 py-3">
          <div className="flex items-center justify-between">
            <span className="font-['Space_Grotesk'] text-[0.6rem] tracking-[0.06em] uppercase text-[var(--color-secondary)]">Load</span>
            <span className="text-[0.8rem] font-medium text-[var(--color-primary)]">{formatDate(row.fdLoadDate)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-['Space_Grotesk'] text-[0.6rem] tracking-[0.06em] uppercase text-[var(--color-secondary)]">ETD</span>
            <span className="text-[0.8rem] font-medium text-[var(--color-primary)]">{formatDate(row.fdETD)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-['Space_Grotesk'] text-[0.6rem] tracking-[0.06em] uppercase text-[var(--color-secondary)]">ETA</span>
            <span className="text-[0.8rem] font-medium text-[var(--color-primary)]">{formatDate(row.fdETA)}</span>
          </div>
          <div className="flex items-center justify-between pt-1.5 mt-1 border-t border-[#EFEDE7]">
            <span className="font-['Space_Grotesk'] text-[0.6rem] tracking-[0.06em] uppercase text-[var(--color-secondary)]">Exit</span>
            <ExitBadge exitDate={row.fdExitDate} />
          </div>
        </div>
      </div>
      ...
      {/* Footer */}
      <div className="px-4 pb-4 pt-3 mt-auto flex gap-2">
        <button
          onClick={() => onViewManifest(row)}
          className="flex-1 inline-flex justify-center items-center gap-1.5 border border-[#E4E1DA] bg-white text-[var(--color-primary)] rounded-[24px] py-[10px] font-['Public_Sans'] text-[0.8rem] font-semibold hover:bg-[#F7F5F2] transition-colors"
        >
          <ClipboardList className="h-3.5 w-3.5" />
          Manifest
        </button>
        <button
          onClick={() => onView(row)}
          className="flex-1 flex items-center justify-center gap-[6px] border border-[var(--color-tertiary)] bg-white text-[var(--color-tertiary)] rounded-[24px] py-[10px] font-['Public_Sans'] text-[0.8rem] font-semibold hover:bg-[#F3E4E0] transition-colors"
        >
  ...
  return (
    <div className="border-b border-[#E4E1DA] last:border-0">
      ...
      >
        ...
        <span className="flex flex-col items-end sm:flex-row sm:items-center sm:gap-1 text-[11.5px] font-medium text-[var(--color-secondary)] mt-1 sm:mt-0 text-right">
            ...
            <>
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full min-w-[1000px] border-collapse bg-white">
                  <thead>
                    <tr className="text-left font-[var(--font-label)] text-[11px] tracking-[0.08em] uppercase text-[var(--color-secondary)] border-b border-[#E4E1DA] bg-slate-50/50">
              ...
              {/* Mobile Grid View */}
              <div className="sm:hidden grid grid-cols-1 gap-3 p-3 bg-slate-50/50">
                {rows.length === 0 ? (
                  <div className="py-6 text-center text-sm text-[var(--color-secondary)] font-medium bg-white rounded-[8px] border border-[#E4E1DA]">
                    Tidak ada data yang ditemukan.
                  </div>
                ) : (
                  rows.map((row) => (
                    <BatchCard key={row.fdMarkingCode} row={row} onView={onView} onViewManifest={onViewManifest} />
                  ))
                )}
              </div>
                ...
                <div className="border-t border-[#E4E1DA] px-3 sm:px-6 py-3 sm:py-4 flex flex-col md:flex-row justify-between items-center gap-3 sm:gap-4 bg-white">
                  ...
                  <div className="flex items-center gap-1.5 text-[12.5px] text-[var(--color-secondary)] shrink-0">
                    ...
                    <select
                      ...
                      className="text-[12.5px] text-[var(--color-primary)] border border-[#E4E1DA] rounded-[var(--radius-sm)] px-2 py-1 outline-none bg-white cursor-pointer font-[var(--font-body)]"
                    >
  ...
  if (isLoading) {
    return (
      <div className="py-16 flex flex-col justify-center items-center bg-[var(--color-surface)] gap-4">
        <span className="w-10 h-10 border-4 border-[var(--color-tertiary)] border-t-transparent rounded-full animate-spin" />
        <p className="text-[var(--color-secondary)] text-sm animate-pulse">Memuat data manifest...</p>
      </div>
    )
  }

  return (
    <div className="flex flex-col min-h-0 bg-[var(--color-surface)]">
      <div className="px-[28px] py-[14px] border-b border-[#E4E1DA] bg-[var(--color-surface)] flex justify-between items-center gap-4 flex-wrap sm:flex-nowrap">
        <div className="relative w-full sm:max-w-[300px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-[9px] bg-white border border-[#E4E1DA] rounded-[var(--radius-md)] text-[0.82rem] text-[var(--color-primary)] placeholder:text-[#A1A1AA] focus:outline-none focus:border-[var(--color-tertiary)] transition-colors shadow-sm"
          />
        </div>
      ...
      <div className="max-h-[420px] overflow-y-auto bg-[#F8FAFC]">
        {filteredManifest.length === 0 ? (
          ...
          <table className="w-full border-collapse bg-white">
            <thead>
              <tr>
                <th className="sticky top-0 bg-[var(--color-neutral)] font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] text-left py-[12px] px-[28px] border-b border-[#E4E1DA] font-medium z-10 w-[15%]">Customer</th>
                <th className="sticky top-0 bg-[var(--color-neutral)] font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] text-left py-[12px] px-[28px] border-b border-[#E4E1DA] font-medium z-10 w-[25%]">Marking / Receiver</th>
                <th className="sticky top-0 bg-[var(--color-neutral)] font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] text-left py-[12px] px-[28px] border-b border-[#E4E1DA] font-medium z-10 w-[20%]">Commodity</th>
                <th className="sticky top-0 bg-[var(--color-neutral)] font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] text-left py-[12px] px-[28px] border-b border-[#E4E1DA] font-medium z-10 w-[25%]">Description</th>
                <th className="sticky top-0 bg-[var(--color-neutral)] font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] text-right py-[12px] px-[28px] border-b border-[#E4E1DA] font-medium z-10 w-[15%]">Summary</th>
              </tr>
            </thead>
            <tbody>
              {filteredManifest.map((m) => (
                <tr key={m.fdListCode} className="hover:bg-[#FBFAF8] group border-b border-[#EFEDE7] last:border-0">
                  <td className="py-[14px] px-[28px] text-[0.85rem] align-top">
                    <span className="font-semibold text-[var(--color-primary)]">{m.fdCustName || '—'}</span>
                  </td>
                  <td className="py-[14px] px-[28px] text-[0.85rem] align-top">
                    <div className="flex flex-col gap-0.5">
                      <div className="flex flex-col gap-0.5">
                        {m.fdMarkingCode && <span className="font-semibold text-[var(--color-primary)] uppercase leading-tight">{m.fdMarkingCode}</span>}
                        {m.fdMarkingNo && <span className="text-[13px] font-semibold text-[var(--color-primary)] uppercase leading-tight">{m.fdMarkingNo}</span>}
                        {!m.fdMarkingCode && !m.fdMarkingNo && <span className="font-medium text-slate-400 leading-tight">-</span>}
                      </div>
                      {m.fdTerima && <span className="text-[13px] text-[var(--color-secondary)] font-medium mt-1 leading-snug">{m.fdTerima}</span>}
                      {m.fdListCode && <span className="text-[13px] text-[var(--color-tertiary)] font-medium leading-snug">{m.fdListCode}</span>}
                    </div>
                  </td>
                  <td className="py-[14px] px-[28px] text-[0.85rem] align-top">
            ...
            {filteredManifest.map((m) => (
              <div key={m.fdListCode} className="bg-[var(--color-surface)] border border-[#E4E1DA] rounded-[8px] overflow-hidden hover:shadow-md transition-all flex flex-col">
                <div className="px-[20px] py-[16px] flex items-center justify-between bg-[var(--color-neutral)] border-b border-[#E4E1DA]">
                  <div>
                    <div className="font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mb-[3px]">List No.</div>
                    <div className="font-[var(--font-display)] font-semibold text-[1.1rem] tracking-[-0.01em] text-[var(--color-primary)]">{m.fdListCode}</div>
                  </div>
                </div>
                <div className="px-[20px] py-[16px] flex flex-col gap-4">
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      <div className="font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mb-1">Customer</div>
                      <div className="font-semibold text-[13px] text-[var(--color-primary)]">{m.fdCustName || '—'}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mb-1">Resi</div>
                      <div className="font-semibold text-[13px] text-[var(--color-primary)]">{m.fdTerima || '-'}</div>
                    </div>
                  </div>
                  <div>
                    <div className="font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mb-1">Marking</div>
                    <div className="font-semibold text-[13px] text-[var(--color-primary)]">{m.fdMarkingCode || m.fdMarkingNo || '-'}</div>
                  </div>
                  <div>
                    <div className="font-[var(--font-label)] text-[0.65rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mb-1">Commodity</div>
                    <div className="flex flex-col gap-1.5 mb-1.5">
                      <div className="font-semibold text-[13px] text-[var(--color-primary)]">{m.fdComodity || '-'}</div>
                      {(() => {
                        const comodityInfo = getCommodityIcon((m as any).fdComodityName || m.fdComodity);
                        const Icon = comodityInfo.Icon;
                        return (
                    ...
                    <div className="text-[12px] text-[var(--color-secondary)] mt-0.5">{m.fdDesc}</div>
                  </div>
                </div>
                <div className="mt-auto px-[20px] py-[14px] bg-[#F7F5F2] border-t border-[#E4E1DA] flex justify-between items-center text-[12.5px]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center px-[28px] py-[14px] bg-[var(--color-neutral)] border-t border-[#E4E1DA] text-[0.82rem] text-[var(--color-secondary)] gap-2">
        <div>Total Manifest: <b className="text-[var(--color-primary)]">{filteredManifest.length}</b></div>
        <div className="flex gap-[24px]">
          <div>Packages: <b className="text-[var(--color-tertiary)]">{totalPkgs.toLocaleString()}</b></div>
          <div>Weight: <b className="text-[var(--color-primary)]">{totalWeight.toLocaleString(undefined, { maximumFractionDigits: 2 })} KG</b></div>
          <div>Volume: <b className="text-[var(--color-primary)]">{totalVol.toLocaleString(undefined, { maximumFractionDigits: 4 })} M3</b></div>
        </div>
      </div>

      <div className="flex justify-end px-[28px] py-[20px] bg-[var(--color-surface)]">
        <button
          onClick={onClose}
          className="border border-[var(--color-primary)] bg-[var(--color-surface)] text-[var(--color-primary)] rounded-[var(--radius-md)] px-[26px] py-[11px] font-[var(--font-body)] font-semibold text-[0.88rem] hover:bg-[var(--color-primary)] hover:text-[var(--color-on-primary)] transition-colors cursor-pointer"
        >
          Close
        </button>
      </div>
    ...
    <div className="flex flex-col gap-4 lg:gap-8 bg-[#F8FAFC] p-3 sm:p-4 lg:p-8 min-h-full">
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-4">
          ...
          >
            ...
            <div className="flex items-center gap-2 text-[var(--color-secondary)]">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider truncate">Prediksi Exit Perlu Perhatian</span>
            </div>
            <div className="mt-3 flex justify-between items-end">
              {isLoadingKpi ? (
                <div className="h-8 w-16 bg-[var(--color-border)] animate-pulse rounded"></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          >
            ...
            <div className="flex items-center gap-2 text-[var(--color-secondary)]">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider">Exit Kemarin</span>
            </div>
            <div className="mt-3 flex justify-between items-end">
              {isLoadingKpi ? (
                <div className="h-8 w-16 bg-[var(--color-border)] animate-pulse rounded"></div>
          ...
          >
            ...
            <div className="flex items-center gap-2 text-[var(--color-secondary)]">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider">Exit Hari Ini</span>
            </div>
            <div className="mt-3 flex justify-between items-end">
              {isLoadingKpi ? (
                <div className="h-8 w-16 bg-[var(--color-border)] animate-pulse rounded"></div>
          ...
          >
            ...
            <div className="flex items-center gap-2 text-[var(--color-secondary)]">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider">Perkiraan Exit Besok</span>
            </div>
            <div className="mt-3 flex justify-between items-end">
              {isLoadingKpi ? (
                <div className="h-8 w-16 bg-[var(--color-border)] animate-pulse rounded"></div>
        ...
        <div className="bg-[var(--color-surface)] rounded-[var(--radius-lg)] border border-[#E4E1DA] p-4 sm:p-6 mb-4 flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
              ...
              <button
                ...
                className="px-[14px] sm:px-[18px] py-[9px] sm:py-[10px] bg-[var(--color-surface)] hover:border-[var(--color-tertiary)] hover:text-[var(--color-tertiary)] text-[var(--color-primary)] text-[13px] sm:text-[13.6px] font-semibold rounded-[var(--radius-md)] border border-[#E4E1DA] transition-colors whitespace-nowrap"
              >
                Cari Manifest
              </button>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-[#E4E1DA] flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
            <span className="font-[var(--font-label)] text-[11px] tracking-[0.08em] uppercase text-[var(--color-secondary)] w-[70px] shrink-0 flex items-center gap-1.5">
        ...
        >
          ...
          >
            <div className="px-6 py-4 border-b border-[var(--color-border)] flex items-center justify-between">
              <h2 className="text-lg font-bold font-[var(--font-display)] text-[var(--color-primary)]">Cari Manifest</h2>
              <button
                onClick={() => setIsCodeModalOpen(false)}
      ...
      {selectedRow && typeof document !== 'undefined' && createPortal(
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 pointer-events-none">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-[#F7F5F2] border border-slate-100 px-2 py-0.5 text-[11px] font-[var(--font-label)] font-medium text-[var(--color-secondary)]">
            ...
            <div className="flex px-5 sm:px-8 border-b border-slate-100 bg-white sticky top-[79px] z-10 text-sm shrink-0">
              <button 
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        {selectedMarking.fdGudang && selectedMarking.fdGudang.trim() !== '' && (
                          <div className="col-span-1 sm:col-span-2 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm">
                            <p className="text-[10px] uppercase tracking-wider font-[var(--font-label)] text-[var(--color-secondary)]">Gudang</p>
                        ...
                        {selectedMarking.fdKet && selectedMarking.fdKet.trim() !== '' && (
                          <div className="col-span-1 sm:col-span-2 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm">
                            <p className="text-[10px] uppercase tracking-wider font-[var(--font-label)] text-[var(--color-secondary)]">Keterangan</p>
                            <p className="mt-1 text-sm font-semibold text-[var(--color-primary)]">
                      ...
                      <div className={cn("grid grid-cols-1 gap-3", listTypeFilter === '2' ? "sm:grid-cols-3" : "sm:grid-cols-2")}>
                        <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-3 text-center shadow-sm">
                          ...
                          <p className="mt-1 text-lg font-bold text-[var(--color-tertiary)]">
                      ...
                      {/* Timestamps */}
                      <div className="flex flex-col sm:flex-row justify-between items-center text-[10px] text-[var(--color-secondary)] gap-2 mt-4 pt-4 border-t border-[var(--color-border)]">
      ...
      {selectedManifestRow && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          >
            {/* Header Panel */}
            <div className="flex items-center justify-between px-[28px] py-[20px] border-b border-[#E4E1DA] bg-[var(--color-surface)] sticky top-0 z-10">
              <div className="flex items-center gap-[14px]">
                <div className="w-[38px] h-[38px] rounded-[var(--radius-md)] border border-[#E4E1DA] bg-[var(--color-neutral)] flex items-center justify-center text-[1.1rem] text-[var(--color-tertiary)]">
                  <ClipboardList className="h-5 w-5" />
                </div>
                <div>
                  <div className="font-[var(--font-display)] font-semibold text-[1.3rem] tracking-[-0.01em] leading-tight text-[var(--color-primary)]">
                    {selectedManifestRow.fdMarkingCode}
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="font-[var(--font-label)] text-[0.7rem] tracking-[0.08em] uppercase text-[var(--color-secondary)] mt-[2px] leading-tight">
                      Batch Manifest List
                    </div>
                    {manifestBatchDetail?.fdContNo && (
                      <>
                        <span className="text-[var(--color-border)] text-xs mt-[2px]">•</span>
                        <div className="font-[var(--font-label)] text-[0.7rem] tracking-[0.08em] uppercase text-[var(--color-tertiary)] mt-[2px] leading-tight font-bold bg-[var(--color-tertiary)]/10 px-1.5 py-0.5 rounded-md">
              ...
              <button
                onClick={() => setSelectedManifestRow(null)}
                className="text-[1.2rem] text-[var(--color-secondary)] hover:text-[var(--color-primary)] transition-colors cursor-pointer"
              >
                <X size={20} />
              </button>
            </div>
